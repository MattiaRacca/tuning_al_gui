...
class Learner(ABC):
    def __init__(self, domain, value_distribution, safe=False, safe_phi=0.0, profiling=False):
        self.domain = domain
    ...
    def safe_phi(self, safe_phi):
        if self.safe:
            ...
            self.tau_cutoff = compute_distribution_tail_cutoff(self.value_distribution, self._safe_phi)
    ...
    def __copy__(self):
        copied_learner = type(self)(domain=copy(self.domain), value_distribution=copy(self.value_distribution))
...
class MinMaxLearner(Learner):
    def __init__(self, n_evidence_minmax, logistic_k_minmax=50.0, *args, **kwargs):
    ...
    def value_distribution(self, value_distribution):
        super(MinMaxLearner, type(self)).value_distribution.fset(self, value_distribution)
    ...
    def reset_learner(self):
        ...
        try:
            ...
            self.max_distribution = compute_sample_max_distribution(self.value_distribution, self.n_evidence_minmax)
            self.min_distribution = compute_sample_min_distribution(self.value_distribution, self.n_evidence_minmax)
    ...
    def update_model(self, answer, query=None):
        ...
        with timer:
            ...
            if answer == LearnerAnswers.FINE:
                self.max_distribution = simple_query_filter(self.domain, self.max_distribution, q,
                                                            self.logistic_k_minmax, True)
                self.min_distribution = simple_query_filter(self.domain, self.min_distribution, q,
                                                            self.logistic_k_minmax, False)
                if self.profiling:
                    max_filter_test = simple_query_filter(self.domain, max_filter_test, q, self.logistic_k_minmax,
                                                          True)
                    min_filter_test = simple_query_filter(self.domain, min_filter_test, q, self.logistic_k_minmax,
                                                          False)
    ...
    def __copy__(self):
        copied_learner = type(self)(domain=copy(self.domain),
                                    value_distribution=copy(self.value_distribution),
...
class RandomMinMaxLearner(MinMaxLearner):
    def choose_query(self):
        ...
        with timer:
            if self.safe:
                ...
                index = safe_selection(self.value_distribution, fake_score, self.tau_cutoff)
            else:
    ...
    def update_model(self, answer, query=None):
        if not self.profiling:
            ...
            update_results = super(RandomMinMaxLearner, self).update_model(answer, query)
            return update_results
    ...
    def choose_query(self):
        ...
        with timer:
            if self.safe:
                ...
                index = safe_selection(self.value_distribution, scores, self.tau_cutoff, score_function=np.argmin)
            else:
    ...
    def choose_query(self):
        ...
        with timer:
            ...
            for j, value in enumerate(self.domain):
                ere_max[j] = compute_expected_reduction_in_entropy(self.domain, self.max_distribution, value)
                ere_min[j] = compute_expected_reduction_in_entropy(self.domain, self.min_distribution, value)

            if self.safe:
                index_max_prior = safe_selection(self.value_distribution, ere_max - entropy_max_prior, self.tau_cutoff,
                                                 score_function=np.argmin)
                index_min_prior = safe_selection(self.value_distribution, ere_min - entropy_min_prior, self.tau_cutoff,
                                                 score_function=np.argmin)
                ...
                if np.min(ere_max - entropy_max_prior) <= np.min(ere_min - entropy_min_prior):
...
class EntropySumMinMaxLearner(MinMaxLearner):
    def choose_query(self):
        ...
        with timer:
            n = self.domain.shape[0]
            ere_sum = np.zeros(n)
            entropy_sum_prior = entropy_categorical(self.min_distribution) + entropy_categorical(self.max_distribution)

            for j, value in enumerate(self.domain):
                ere_sum[j] += compute_expected_reduction_in_entropy(self.domain, self.max_distribution, value)
                ere_sum[j] += compute_expected_reduction_in_entropy(self.domain, self.min_distribution, value)

            if self.safe:
                index = safe_selection(self.value_distribution, ere_sum - entropy_sum_prior, self.tau_cutoff,
                                       score_function=np.argmin)
            else:
...
class DivergenceMinMaxLearner(MinMaxLearner):
    def choose_query(self):
        ...
        with timer:
            ...
            for j, value in enumerate(self.domain):
                erd_max[j] = compute_expected_reduction_in_divergence(self.domain, self.max_distribution, value)
                erd_min[j] = compute_expected_reduction_in_divergence(self.domain, self.min_distribution, value)

            if self.safe:
                index_max_prior = safe_selection(self.value_distribution, erd_max, self.tau_cutoff,
                                                 score_function=np.argmax)
                index_min_prior = safe_selection(self.value_distribution, erd_min, self.tau_cutoff,
                                                 score_function=np.argmax)
                index = index_max_prior if erd_max[index_max_prior] >= erd_min[index_min_prior] else index_min_prior
            else:
...
class RangeLearner(Learner):
    def __init__(self, n_evidence_range, logistic_k_range=20.0, *args, **kwargs):
    ...
    @Learner.value_distribution.setter
    def value_distribution(self, value_distribution):
        super(RangeLearner, type(self)).value_distribution.fset(self, value_distribution)
        # (re)compute related distributions
        self.reset_learner()
    ...
    def reset_learner(self):
        ...
        try:
            ...
            self.range_distribution = compute_sample_range_distribution(self.value_distribution,
                                                                        self.n_evidence_range)
    ...
    def update_model(self, answer, query=None):
        ...
        with timer:
            ...
            if answer == LearnerAnswers.FINE:
                self.range_distribution = range_query_filter(self.domain, self.range_distribution, q,
                                                             self.logistic_k_range, is_range=True)
                if self.profiling:
                    range_filter_test = range_query_filter(self.domain, range_filter_test, q, self.logistic_k_range)
            elif answer == LearnerAnswers.HIGHER:
                self.range_distribution = range_query_filter(self.domain, self.range_distribution, q,
                                                             self.logistic_k_range, higher_max=True,
                                                             higher_min=True, is_range=True)
                if self.profiling:
                    range_filter_test = range_query_filter(self.domain, range_filter_test, q, self.logistic_k_range,
                                                           higher_max=True, higher_min=True)
            elif answer == LearnerAnswers.LOWER:
                self.range_distribution = range_query_filter(self.domain, self.range_distribution, q,
                                                             self.logistic_k_range, higher_max=False, higher_min=False,
                                                             is_range=True)
                if self.profiling:
                    range_filter_test = range_query_filter(self.domain, range_filter_test, q, self.logistic_k_range,
                                                           higher_max=False, higher_min=False)
            else:
                raise LearnerException(1)
    ...
    def get_current_estimate(self):
        indexes = np.unravel_index(np.argmax(self.range_distribution, axis=None), self.range_distribution.shape)
        return [self.domain[indexes[1]], self.domain[indexes[0]]]  # the max is on the x-axis...
...
class EntropyRangeLearner(RangeLearner):
    def choose_query(self):
        ...
        with timer:
            ...
            for j, value in enumerate(self.domain):
                ere_values[j] = compute_expected_reduction_in_entropy(self.domain, self.range_distribution, value)
            if self.safe:
                index = safe_selection(self.value_distribution, ere_values, self.tau_cutoff, score_function=np.argmin)
            else:
...
class DivergenceRangeLearner(RangeLearner):
    def choose_query(self):
        ...
        with timer:
            ...
            for j, value in enumerate(self.domain):
                divergences[j] = compute_expected_reduction_in_divergence(self.domain, self.range_distribution, value)

            if self.safe:
                index = safe_selection(self.value_distribution, divergences, self.tau_cutoff)
            else:
...
def simple_query_filter(domain, function, current_value, logistic_k, higher=True, pdf=True):
...
def range_query_filter(domain, function, current_value, logistic_k, higher_max=True, higher_min=False, pdf=True,
                       is_range=False):
    ...
    max_filter = simple_query_filter(domain, np.ones(function.shape[0]), current_value, logistic_k, higher_max)
    min_filter = simple_query_filter(domain, np.ones(function.shape[1]), current_value, logistic_k, higher_min)
...
def is_true_categorical_pdf(categorical_pdf, strict=False):
    ...
    try:
        ...
        if strict:
            print('pdf integral not summing to 1 but to {}...'.format(np.sum(categorical_pdf)))
            raise
        else:
    ...
    try:
        ...
        categorical_pdf[np.where(np.isclose(categorical_pdf[np.where(categorical_pdf < 0.0)], 0.0))] = 0.0
        assert np.alltrue(categorical_pdf[np.where(np.invert(np.isclose(categorical_pdf, 0.0)))] >= 0)
    except AssertionError:
        print('pdf contains negative probabilities...')
        print('apparently pdf contains has {}'.format(categorical_pdf[np.where(categorical_pdf < 0.0)]))
        raise
    return True
...
def entropy_categorical(categorical_pdf):
    ...
    entropy = np.sum([-value*np.log2(value) for value in categorical_pdf
...
def kl_divergence_categorical(first_pdf, second_pdf):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        return np.sum(np.where(first_pdf < np.finfo(first_pdf.dtype).eps, 0.0,
...
def js_divergence_categorical(first_pdf, second_pdf):
    ...
    return 0.5 * (kl_divergence_categorical(first_pdf, sum_pdf) + kl_divergence_categorical(second_pdf, sum_pdf))
...
def numerical_integration(domain, function, lower_limit, upper_limit):
    if len(function.shape) == 1:
        ...
        if i_x_lower is None or i_x_upper is None or i_y_lower is None or i_y_upper is None:
            print('Cannot find lower or upper bounds')
            raise ValueError
        return np.sum(function[i_x_lower:i_x_upper + 1, i_y_lower:i_y_upper + 1])
    else:
...
def compute_expected_reduction_in_entropy(domain, function, query_value):
    if len(function.shape) == 1:
        posterior_higher = simple_query_filter(domain, function, query_value, 50.0, True)
        posterior_lower = simple_query_filter(domain, function, query_value, 50.0, False)

        probs = np.zeros(2)
        probs[0] = numerical_integration(domain, function, query_value, domain[-1])  # Higher
        probs[1] = numerical_integration(domain, function, domain[0], query_value)  # Lower
        ...
        posterior_fine = range_query_filter(domain, function, query_value, 20.0)
        posterior_higher = range_query_filter(domain, function, query_value, 20.0, higher_max=True, higher_min=True)
        posterior_lower = range_query_filter(domain, function, query_value, 20.0, higher_max=False, higher_min=False)

        probs = np.zeros(3)
        # in the order, Fine, Higher, Lower
        probs[0] = numerical_integration(domain, function, [query_value, domain[0]], [domain[-1], query_value])
        probs[1] = numerical_integration(domain, function, [query_value, query_value], [domain[-1], domain[-1]])
        probs[2] = numerical_integration(domain, function, [domain[0], domain[0]], [query_value, query_value])
        probs /= np.sum(probs)
        ...
        print('Can only do this for up to 2 dimensions, not for {}'.format(len(function.shape)))
        raise ValueError


def compute_expected_reduction_in_divergence(domain, function, query_value, method='js'):
    if method == 'js':
        chosen_method = js_divergence_categorical
    elif method == 'kl':
        chosen_method = kl_divergence_categorical
    else:
        print('Unknown divergence method. Known methods: Jensen-Shannon js, Kullback-Leibler kl.\n' +
...
def compute_sample_range_distribution(pdf, n_evidence):
    ...
    for u in range(0, pdf.shape[0]):
        for v in range(0, pdf.shape[0]):
            range_pdf[u, v] = n_evidence * (n_evidence - 1) * (cdf[u] - cdf[v]) ** (n_evidence - 2) * pdf[u] * pdf[v]
...
def safe_selection(distribution, score, distribution_cutoff, cutoff_function=np.greater_equal,
                   score_function=np.argmax):
...
def generate_prior(seed, domain, beta_weights=[2,10]):
    ...
    first_prior = beta(np.random.randint(beta_weights[0], beta_weights[1]), np.random.randint(beta_weights[0], beta_weights[1]))
    second_prior = beta(np.random.randint(beta_weights[0], beta_weights[1]), np.random.randint(beta_weights[0], beta_weights[1]))
    third_prior = beta(np.random.randint(beta_weights[0], beta_weights[1]), np.random.randint(beta_weights[0], beta_weights[1]))
    forth_prior = beta(np.random.randint(beta_weights[0], beta_weights[1]), np.random.randint(beta_weights[0], beta_weights[1]))
    w = np.random.dirichlet((1, 1, 1, 1))

    prior = []
    for value in domain:
        prior.append(w[0]*first_prior.pdf(value) + w[1]*second_prior.pdf(value) + w[2]*third_prior.pdf(value) + w[3]*forth_prior.pdf(value))
    ...
    filtered_upperbound_prior = simple_query_filter(domain, upperbound_prior, lowerbound + bin_size, 1000.0, True)
    upperbound = domain[np.nonzero(np.random.multinomial(1, filtered_upperbound_prior))]
